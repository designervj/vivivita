...
import { TESTIMONIALS_CONTENT, TESTIMONIAL_SLIDES, type HomeContent } from '../../../lib/pages/content';
...
export default function TestimonialsSection({ slides = TESTIMONIAL_SLIDES, content = TESTIMONIALS_CONTENT }: { slides?: HomeContent['testimonialSlides']; content?: HomeContent['testimonialsContent'] }) {
  const [currentSlideIdx, setCurrentSlideIdx] = useState(0);

  const totalSlides = slides.length;
  const currentSlide = slides[currentSlideIdx] || TESTIMONIAL_SLIDES[0];
    ...
    <section id="iskustva" className="pt-8 sm:pt-12 pb-16 sm:pb-20 container mx-auto px-3 sm:px-6 md:px-8">
      ...
      <div className="mb-12 sm:mb-[58px]">
        <span className="text-[var(--primary)] text-[11px] sm:text-[16px] font-medium tracking-normal block mb-3">
          <EditableText sectionId="testimonials-001" fieldPath="tag.hr" value={content.tag} />
        </span>
            ...
            {currentSlide.quotes.map((item) => (
              <div key={item.id} className="max-w-4xl">
                <blockquote className="text-neutral-800 text-[14px] sm:text-[16px] leading-[1.6] sm:leading-[1.55] font-sans font-semibold">
                  “<EditableText sectionId="testimonials-001"  fieldPath={`slides.${currentSlideIdx}.quotes.${currentSlide.quotes.findIndex((quote) => quote.id === item.id)}.quote`} value={item.quote} />”
                </blockquote>
                <div className="text-[12px] sm:text-[16px] text-neutral-500 font-sans mt-3 sm:mt-4">
                  <EditableText sectionId="testimonials-001" fieldPath={`slides.${currentSlideIdx}.quotes.${currentSlide.quotes.findIndex((quote) => quote.id === item.id)}.author`} value={item.author} />,
                  {' '}
                  <EditableText sectionId="testimonials-001" fieldPath={`slides.${currentSlideIdx}.quotes.${currentSlide.quotes.findIndex((quote) => quote.id === item.id)}.age`} value={item.age} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center pt-8 sm:pt-10 max-w-[420px] w-full lg:ml-[-220px]">
            <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none px-0 sm:px-[48px] py-[12px] sm:py-[10px] rounded-full bg-[var(--secondary)] hover:bg-[var(--primary-soft)] active:scale-95 flex items-center justify-center text-[var(--primary)] transition-colors cursor-pointer"
              >
            ...
            <span className="text-[11px] font-sans italic mt-2 font-[600] text-[var(--muted-foreground)]">
